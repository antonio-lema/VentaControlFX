...
public class PromotionsController implements Injectable {
    ...
    private void handleDelete(Promotion promotion) {
        ...
        if (com.mycompany.ventacontrolfx.presentation.util.AlertUtil.showConfirmation(title, header, content)) {
            try {
    ...
    private void showPromotionForm(Promotion promotion) {
        try {
            PromotionFormController controller = com.mycompany.ventacontrolfx.presentation.navigation.ModalService.showTransparentModal(
                    "/view/dialog/promotion_form.fxml",
