...
public class StaffCalendarController implements Injectable {
    ...
    private void renderCalendar() {
        ...
        lblMonthYear.setText(
                viewingDate.getMonth().getDisplayName(TextStyle.FULL, Locale.forLanguageTag("es")).toUpperCase() + " "
                        + viewingDate.getYear());
    ...
    private VBox createDayCell(LocalDate date) {
        ...
        if (bd != null && !bd.isClosed() && (!special.isPresent() || !special.get().isClosed())) {
            ...
            if (!assignedIdSet.isEmpty()) {
                HBox staffBadge = createBadge(assignedIdSet.size() + " EMPLEADOS", "#059669", FontAwesomeIcon.USERS);
        ...
        for (StaffVacation vac : activeVacations) {
            String userName = allUsers.stream().filter(u -> u.getUserId() == vac.getUserId())
                    .map(User::getFullName).findFirst().orElse("User " + vac.getUserId());
            HBox vacBadge = createBadge(userName.split(" ")[0] + " (VAC)", "#0284c7", FontAwesomeIcon.UMBRELLA);
