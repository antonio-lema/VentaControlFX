...
public class StatusBarController implements Injectable, GlobalEventBus.DataChangeListener, GlobalEventBus.VerifactuSyncListener {
    ...
    public void onSyncStarted() {
        ...
        updateLabel(lblVerifactuStatus, container.getBundle().getString("verifactu.status.processing"));
    }

    @Override
    public void onSyncFinished(String result) {
        if ("OK".equals(result)) {
            updateLabel(lblVerifactuStatus, container.getBundle().getString("verifactu.status.success"));
            if (badgeVerifactu != null) {
            ...
            updateLabel(lblVerifactuStatus, container.getBundle().getString("verifactu.status.error") + " (" + result + ")");
            if (badgeVerifactu != null) {
